...
class ReviewWebSocketConsumer(AsyncJsonWebsocketConsumer):
    ...
    def get_random_card(self):
        cards=list(Card.objects.filter(deck=self.deck,due__lte=timezone.now()))
        if cards:
            self.card=random.choice(cards)
            return
        cards=list(Card.objects.filter(deck=self.deck,due=None))
        if cards:
            self.card=random.choice(cards)
            return
        cards=list(Card.objects.filter(deck=self.deck))
        self.card=random.choice(cards) if cards else None
